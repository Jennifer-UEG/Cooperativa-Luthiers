...
import { IsDate, IsNotEmpty, IsNumber, IsBoolean, Min, Max, IsString, IsDateString } from 'class-validator';

export class CreateInstrumentoDto {
    @ApiProperty({ description: 'Modelo e madeira do instrumento', example: 'Violão de nylon' })
    @IsString()
    @IsNotEmpty({ message: 'O modelo/madeira do instrumento é obrigatório' })
    modeloMadeira: string;

    @ApiProperty({ description: 'Data de entrada do instrumento na oficina', example: '2022-01-01' })
    @IsDateString()
    ...
    @ApiProperty({ description: 'ID do luthier responsável pelo reparo', example: 1 })
    @IsNumber()
    @IsNotEmpty({ message: 'O ID do luthier é obrigatório' })
    luthierId: number;

    @ApiProperty({ description: 'Custo do reparo', example: 1250.20 })
    @IsNumber()
