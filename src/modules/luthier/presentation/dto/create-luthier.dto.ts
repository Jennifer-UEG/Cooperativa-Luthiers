...
import { IsString, IsBoolean, IsNumber, IsDateString, IsNotEmpty } from "class-validator";
...
export class CreateLuthierDto {
    ...
    @ApiProperty({ example: "2022-01-01", description: 'Data de abertura da oficina (YYYY-MM-DD)' })
    @IsDateString({}, { message: 'A data de abertura deve ser uma data válida no formato YYYY-MM-DD' })
    @IsNotEmpty({ message: 'A data de abertura é obrigatória' })
    dataAbertura: Date;

    @ApiProperty({ example: true, description: 'Indica se a oficina possui certificação profissional' })
    @IsBoolean()
    certificada: boolean;
